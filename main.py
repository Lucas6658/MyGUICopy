...
from weather import WorkerSignals
import weather
...
class CalendarWindow(QWidget):
    ...
    def addNewTask(self):
        ...
        if newTask == "":
            ...
            messageBox.exec()
            return
        #elif not(bool(re.fullmatch(pattern, newTaskStrip))):
            #messageBox = QMessageBox()
            #messageBox.setText("Text must be plausible.")
            #messageBox.setStandardButtons(QMessageBox.Ok)
            #messageBox.setStyleSheet("QLabel{min-width:300 px; font-size: 24px;} QWidget{background-color:#0000FF; color: black} QPushButton{ width:100px; font-size: 18px; }")
            #messageBox.exec()
            #return
